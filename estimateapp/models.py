...
class EstimateModel(models.Model):
    ...
    def detail(self):
        ...
        #             print(f'{expense_corr}.는 공사비의 {i:.2f}%입니다\n')
        #         elif "산재" in expense_corr:
        #             i = expense / total_expense * 100
        #             print(f'{expense_corr}.는 공사비의 {i:.2f}%입니다')
        #             if total_expense * 110 >= 50000000:
        #                 print("(공사비용이 5천만원이하이므로 적절하지 못합니다)\n")
        #             else:
        #                 print("\n")
        #         elif "고용" in expense_corr:
        #             i = expense / total_expense * 100
        #             print(f'{expense_corr}.는 공사비의 {i:.2f}%입니다')
        #             if total_expense * 110 >= 50000000:
        #                 print("(공사비용이 5천만원이하이므로 적절하지 못합니다)\n")
        ...
        #             i = expense / total_expense * 100
        #             print(f'그 외 추가 비용 검토해보세요: {expense_corr}.는 공사비의 {i:.2f}%입니다.\n')
        # except:
        #     print('평가할 수 없습니다. 검토필요')
        for expense_corr, expense in final_detail_dict.items():
            if expense == total_expense:
                순공사비 = expense_corr
                list_1.append(f'{expense_corr}...ok...평가 가능\n')
                break
        try:
            순공사비
            for expense_corr, expense in final_detail_dict.items():
                if "이윤" in expense_corr:
                    i = expense / total_expense * 100
                    list_1.append(f'{expense_corr}.는 공사비의 {i:.2f}%입니다\n')
                elif "산재" in expense_corr:
                    i = expense / total_expense * 100
                    list_1.append(f'{expense_corr}.는 공사비의 {i:.2f}%입니다')
                    if total_expense * 110 >= 50000000:
                        list_1.append("(공사비용이 5천만원이하이므로 적절하지 못합니다)\n")
                    else:
                        list_1.append("\n")
                elif "고용" in expense_corr:
                    i = expense / total_expense * 100
                    ...
                    i = expense / total_expense * 100
                    list_1.append(f'그 외 추가 비용 검토해보세요: {expense_corr}.는 공사비의 {i:.2f}%입니다.\n')
        except:
            list_1.append('평가할 수 없습니다. 검토필요')
        return list_1
